...
for (let i = 0; i < graduates.length; i++) {
    ...
    let fNameEmail = `${fName} ${email}`;
    contactInfo.push(fNameEmail);
}
...
const unisWithUni = [];
for (let i = 0; i < graduates.length; i++) {
    let uni = graduates[i].university.split(" ");
    for (let u = 0; u < uni.length; u++) {
        if (uni[u] === 'University' || uni[u] === 'Universitat' || uni[u] === 'Universidad') {
...
zooAnimals.map(name => {
    let str = name.animal_name.toLowerCase();
    lowCaseAnimalNames.push(str);
});
...
zooAnimals.reduce((acc, cValue) => {
    return populationTotal = acc + cValue.population;
}, 0);
...
I just naturally used them as Arrow functions lol -- Isaac
*/